...
struct PrecAnalisys24hView: View {
   
    var data = PrecAnalisys3hData()
    ...
    var body: some View {
        ...
        .onChange(of: data.validTimeList) {
            if data.validTimeList.count > 2 {
                ...
                overlay = MKTileOverlay(urlTemplate: "https://www.jma.go.jp/bosai/jmatile/data/rasrf/\(baseTime)/\(type)/\(validTime)/surf/rasrf24h/{z}/{x}/{y}.png")
            }
        }
    }
}

extension PrecAnalisys24hView {
